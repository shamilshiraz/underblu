import React from 'react'

function Footer() {
  return (
    <div className='h-[20vh] p-5 mt-5 w-full bg-swblue flex flex-col justify-around items-center'>
        <svg id="Layer_2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 84.53 85.37" className="h-[50px] ">
          <g id="Layer_1-2">
            <g>
              <path className="fill-[#ffffff]" d="M58.45,85.37l-7.46-.04c-.41,0-.77-.25-.93-.63l-5.6-13.87c-.16-.4-.05-.85.27-1.13l33.38-29.38c.58-.5,1.48-.2,1.64.54,1.48,6.8,3,13.17,3.41,15.8,1.93,12.4-1.65,28.71-24.71,28.71Z"/>
              <path className="fill-[#ffffff]" d="M43.58,84.45l-16.07-.09c-22.48,0-26.17-16.38-24.28-28.56.5-3.2,2.65-12,4.36-20.33.07-.36.34-.65.7-.76l14.03-4.14c.49-.15,1.02.1,1.21.58l15.42,38.18,5.56,13.76c.27.66-.22,1.38-.94,1.37Z"/>
              <path className="fill-[#ffffff]" d="M45.2,1.32l-6.52,20.13c-.1.32-.35.56-.67.65l-15.67,4.65-13.88,4.1c-.72.21-1.41-.42-1.26-1.16.75-3.8,1.37-7.38,1.65-10.15.34-3.49-.02-6.94-2.57-9.69L.22,2.42c-.14-.18-.22-.4-.22-.63,0-.54.42-.99.96-1.01,4.27-.16,21.99-.78,38.75-.78,1.5,0,3.03,0,4.53,0,.68,0,1.16.67.95,1.31Z"/>
              <path className="fill-[#ffffff]" d="M78.25,10.86c-2.54,2.75-2.91,6.2-2.57,9.69.39,3.86,1.43,9.28,2.58,14.66.08.35-.04.72-.31.96l-33.69,29.66c-.52.46-1.33.26-1.59-.38l-14.82-36.68c-.22-.55.07-1.16.64-1.33l14.62-4.34c.32-.09.57-.34.67-.65L50.49,1.71c.13-.42.53-.7.97-.69,14.61.13,28.42.62,32.12.76.54.02.96.46.96,1.01,0,.23-.08.45-.22.63l-6.05,7.44Z"/>
            </g>
          </g>
        </svg>
        
        <div className="flex text-white text-sm">
            <p className='pe-1'>underblu.in</p>
            <p className='px-1 border-x'>underblu@gmail.com</p>
            <p className='ps-1'>+91 8945678978</p>
        </div>
    </div>
  )
}

export default Footer
